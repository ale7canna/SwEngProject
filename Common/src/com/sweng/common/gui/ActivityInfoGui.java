...
public class ActivityInfoGui extends JPanel {
	...
	private JList listResponsible;
	private JTextField textFieldPlace;
	private JTextField textFieldHour;
	private JCheckBox checkBox;
	private JTextPane textPane;
	
	public ActivityInfoGui(ActivityInfo _activityInfo, ICommonGui _listener) {
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent evt) {
				...
				int scelta = JOptionPane.showConfirmDialog(null, "Do you really want add the text to this activity?",
		...
		if (activityInfo.isFinishable()) {
			...
			btnCompleteActivity.addMouseListener(new MouseAdapter() {
				...
				public void mousePressed(MouseEvent evt) {
					...
					int scelta = JOptionPane.showConfirmDialog(null, "Do you really want to complete this activity?",
		...
		gl_panelResponsabili.setHorizontalGroup(gl_panelResponsabili.createParallelGroup(Alignment.TRAILING)
				.addGroup(gl_panelResponsabili.createSequentialGroup().addGap(132).addComponent(lblResponsible)
						.addContainerGap(91, Short.MAX_VALUE))
				.addGroup(gl_panelResponsabili.createSequentialGroup().addContainerGap(84, Short.MAX_VALUE)
						.addComponent(scrollPane_1, GroupLayout.PREFERRED_SIZE, 230, GroupLayout.PREFERRED_SIZE)
						.addGap(60))
				.addGroup(Alignment.LEADING, gl_panelResponsabili.createSequentialGroup().addContainerGap()
						.addComponent(lblNewLabel_1).addContainerGap(180, Short.MAX_VALUE)));
		gl_panelResponsabili.setVerticalGroup(gl_panelResponsabili.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panelResponsabili.createSequentialGroup().addGap(13)
						.addComponent(lblResponsible).addGap(26).addComponent(scrollPane_1, GroupLayout.PREFERRED_SIZE,
								GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
						.addGap(191).addComponent(lblNewLabel_1).addContainerGap()));
		...
		listResponsible.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent ev) {
				...
				if (ev.getClickCount() == 2) {
					if (activityInfo.getProject().isComplete())
						JOptionPane.showMessageDialog(null,
								"You cannot remove the responsible cause the project is complete.");
					else if (activityInfo.isDone())
						JOptionPane.showMessageDialog(null,
								"You cannot remove the responsible cause the activity is complete.");
					else {
						
						User u = (User) ((MyUserListModel) listResponsible.getModel())
								.getUserAt(listResponsible.getSelectedIndex());
						int scelta = JOptionPane.showConfirmDialog(
								null, "Do you really want to remove " + u.getUsername() + " from "
										+ activityInfo.getName() + " activity?",
								"Remove participant", JOptionPane.YES_NO_OPTION);
						if (scelta == JOptionPane.YES_OPTION) {
							
							ActivityResponsible resp = new ActivityResponsible(u.getIdUser(),
									activityInfo.getIdActivity());
	...
	public void refreshContent(ActivityInfo activityInfo, ArrayList<User> responsibles) {
